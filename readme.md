...
❗ 所有图片和视频演示均为v1.0版本，新增功能请在[更新历史](#更新历史)查看，也可以在[参数解释](#参数解释)中找到功能对应参数的详细解释

❗ 若脚本界面中显示中文乱码，请先尝试更新Aegisub到最新版本
...
+ v1.4.0
    ...
    + 优化自动分行功能，现在不会再拆分英文单词，除非使用连字符
    
+ v1.3.0

    + 修改菜单显示顺序，修改菜单部分按钮为中文

    + 新增**自动设置首末空字幕行**的功能
        + 勾选 **提前开始自** 选项，并填写时间，即可在第一行歌词前**自动生成一行空字幕**，此行字幕的开始时间为填写时间，结束时间为原最早开始时间
        + 勾选 **延后结束到** 选项，并填写时间，即可在最后一行歌词后自动生成一行空字幕，此行字幕的开始时间为原最晚结束时间，结束时间为填写时间
        + 点击 **恢复默认延长显示时间** 后，将两个时间分别恢复为 `00:00:00` 和 `99:59:59`
            + Aegisub 没有获取音频时长的功能，因此无法自动设置结束时间为音频结束时间，但使用 `99:59:59` 也可以满足对应需求
        + **时间** 的格式：`hh:mm:ss`，其中 `hh`  和 `mm` 可以省略
            + 示例：
                + 1时5分39秒：`01:05:39`、`1:5:39`
                + 5分39秒：`05:39`、`5:39`
                + 39秒：`39`
...
+ v1.2.3
    ...
    + 修改缩放参数 `\fsc` 为 `\fscx\fscy`

+ v1.2.2

    + 增加**透明度渐变与否**的选项
        + 勾选此选项，对应着之前版本中默认的情况：显示范围内第一组和最后一组字幕呈现**逐渐清晰/逐渐透明效果**

        + 不勾选此选项，则直接在显示范围边界**截断字幕**

    + 修改透明度渐变的计算方式为余弦函数而非线性

    + 修复某些情况下生成滚动字幕会使得Aegisub爆内存的bug
...
| 延长显示时间         | 提前开始时间 | 时间字符串 |                            | 勾选后，在第一行歌词前自动生成一行空字幕，此字幕的开始时间为填写时间，结束时间为原最早开始时间<br />格式：[[mm:]hh:]ss<br />格式示例：<br/>1时5分39秒：`01:05:39`、`1:5:39`<br/>5分39秒：`05:39`、`5:39`<br/>39秒：`39` |
|                      | 延后结束时间 | 时间字符串 |                            | 勾选后，在最后一行歌词后自动生成一行空字幕，此字幕的开始时间为原最晚结束时间，结束时间为填写时间<br />格式同上 |
| 首末组字幕透明度渐变 |              | 布尔型     | 勾选 / 未勾选              | 显示范围内的第一组和最后一组字幕会有透明度渐变效果<br />取消后只做硬裁剪 |
...
| 记忆本次输入的参数   |              | 布尔型     | 勾选 / 未勾选              | 勾选会在插件目录下生成 LyricsRoller_config.txt 以保存参数信息<br />点击 **恢复默认参数** 可恢复最初的默认参数 |
